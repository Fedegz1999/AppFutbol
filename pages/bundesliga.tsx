...
export default function bundesliga(data: any) {
  ...
  const res = await fetch("https://api.unidadeditorial.es/sports/v1/classifications/current/?site=2&type=10&tournament=0108");
   const rank = await res.json();

   const res2 = await fetch("https://api.unidadeditorial.es/sports/v1/player-total-rank/sport/01/tournament/0108/sort/goals/current/?site=2&mn=10")
   const gol = await res2.json();

   const res3 = await fetch("https://api.unidadeditorial.es/sports/v1/player-total-rank/sport/01/tournament/0108/sort/assists/current/?site=2&mn=10")
   const asist = await res3.json();
   
   const res4 = await fetch("https://spoyer.com/api/en/get.php?login=ayna&token=12784-OhJLY5mb3BSOx0O&task=predatapage&sport=soccer&league=123&p=1")
